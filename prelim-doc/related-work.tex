...
%
Note that, even\Comment{check} though Martignoni
...
smallish binaries. For example, out of $22$ binaries under test, \cmt{only $3$
  achieve full byte equivalence when the initial population does not includes
    decompiler seeds, and} $4$ achieve full byte equivalence when the initial
...
include Equivalence checking, \TV, and Software Verification.%, and Semantics
defining frameworks. 
...
%\cmt{With the ultimate goal of translation validation for optimizing
  %compilers,} 
...
%If a variable \m{x1} in the one program have the same values as a variable
%\m{x2} in the optimized version for all possible runs of the two programs on
%the %same input values, then they are defined as corresponding variables.
...
the VC generation from a given correspondence or proving the VCs.  Moreover,
    ...
    correspondence. We note that only the basic blocks \m{b1} and \m{b2}, which
