...
\begin{itemize}
    ...
    the source code or library code. \cmt{However, a common workaround for
      these problems, which already in common practice, is to preprocess input
        ...
        For example, for languages that are compiled to an intermediate form,
            such as Java bytecode, Microsoft's Common Intermediate Language
              (CIL), or LLVM~\cite{Lattner:2004} IR, it is already common
              practice to analyze IR instead of source, in order to avoid
              problems from parsing and to support all the different source
    ...
    hardware~\cite{WYSINWYE}.  \cmt{ \item Binary analysis is heavily leveraged
      by various tools, ranging from software emulation and
        ...
        sand-boxing~\cite{Kiriansky:2002:SEV,Erlingsson:2006,Yee:2009}, and
          ...
          their performance and reliability.} \end{itemize}
