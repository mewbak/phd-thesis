...
the lifted LLVM IR code, we decided to build on K~\cite{k-primer-2013-v32},
    ...
    language semantics (Refer to Section~\ref{sec:related-work} for the other framework options}. Given a
    syntax and a semantics of a language, K generates a parser, an interpreter,
    as well as formal analysis tools such as model checkers and deductive
    program verifiers, at no additional cost. Using the interpreter, one can
    test their semantics immediately, which significantly increases the
    efficiency of semantics developments. Furthermore, the formal analysis
    tools facilitate formal reasoning about the given language semantics. This
    helps in terms of both applicability of the semantics and engineering the
    semantics itself.
...
like the proof generator to work uniformly across translators.   
 ...
 Toward the end to achieve language-independence, we borrowed a
